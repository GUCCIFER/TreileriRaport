...
export default function Home() {
  ...
  // Generates PDF and returns a Blob (does NOT save to disk)
  const generatePDFBlob = async (): Promise<Blob> => {
    ...
    const addFooter = (pageNum: number) => {
      ...
      doc.setFontSize(8);
      doc.text(`Generated by ${formData.companyName} | Linford`, pageWidth / 2, pageHeight - 10, { align: 'center' });
  ...
  const handleSubmit = async () => {
    if (!formData.unitNumber) {
      alert('Please enter a Unit Number before submitting.');
      setActiveTab('info');
      return;
    }
    ...
    try {
      ...
      // 2. Trigger browser download immediately (doesn't wait for server)
      ...
      alert('Report downloaded but could not be saved to the server. Please check your connection.');
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50 to-sky-50">
      ...
      {saveSuccess && (
        <div className="bg-emerald-500 text-white text-center py-3 font-semibold">
          ✅ Report saved and PDF downloaded successfully!
        </div>
      )}
      ...
      <div className="bg-white border-b px-4 py-3">
        <div className="max-w-6xl mx-auto flex gap-6 text-sm flex-wrap">
          ...
          <div className={issueCount > 0 ? 'text-red-600 font-semibold' : 'text-green-600 font-semibold'}>
            {issueCount > 0 ? `⚠️ ${issueCount} Issues Found` : '✅ No Issues'}
          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      title="Inspector name is set from your account"
                    />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
