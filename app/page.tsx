...
export default function Home() {
  ...
  const handlePhotoUpload = (position, e) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        setPhotos(prev => ({ ...prev, [position]: event.target?.result }));
      };
            ...
            {tabs.map(tab => (
              <button
                ...
                className={`px-6 py-4 font-medium flex items-center gap-2 whitespace-nowrap transition-all
                  ${activeTab === tab.id 
                    ? 'bg-white text-sky-700 border-b-3 border-sky-500' 
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {Object.entries(formData.tires).map(([position, tire]) => {
                    ...
                    return (
                      <div key={position} className="grid grid-cols-4 gap-4 items-center p-4 bg-gray-50 rounded-lg">
